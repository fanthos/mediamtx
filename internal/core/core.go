...
func (p *Core) createResources(initial bool) error {
	...
	if p.conf.WebRTC &&
		...
		p.webRTCManager = &webRTCManager{
			...
			Mode:                  p.conf.WebRTCMode,
			PassiveUDPAddress:     p.conf.WebRTCPassiveUDPAddress,
			PassiveTCPAddress:     p.conf.WebRTCPassiveTCPAddress,
...
func (p *Core) closeResources(newConf *conf.Conf, calledByAPI bool) {
	...
	closeWebRTCManager := newConf == nil ||
		...
		newConf.WebRTCMode != p.conf.WebRTCMode ||
		newConf.WebRTCPassiveUDPAddress != p.conf.WebRTCPassiveUDPAddress ||
		newConf.WebRTCPassiveTCPAddress != p.conf.WebRTCPassiveTCPAddress ||
		closeMetrics ||
		closePathManager ||
		closeLogger
