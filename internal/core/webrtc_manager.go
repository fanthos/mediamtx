...
type webRTCManager struct {
	...
	PassiveUDPAddress     string
	PassiveTCPAddress     string
...
func (m *webRTCManager) initialize() error {
	...
	var apiConf webrtc.APIConf

	apiConf.Mode = m.Mode
	apiConf.IPsFromInterfaces = m.IPsFromInterfaces
	apiConf.IPsFromInterfacesList = m.IPsFromInterfacesList
	apiConf.AdditionalHosts = m.AdditionalHosts

	if m.Mode == conf.WebRTCModePassive {
		if m.PassiveUDPAddress != "" {
			m.udpMuxLn, err = net.ListenPacket(restrictnetwork.Restrict("udp", m.PassiveUDPAddress))
			if err != nil {
				m.httpServer.close()
				ctxCancel()
				return err
			...
			apiConf.ICEUDPMux = iceUDPMux
		}

		if m.PassiveTCPAddress != "" {
			m.tcpMuxLn, err = net.Listen(restrictnetwork.Restrict("tcp", m.PassiveTCPAddress))
			if err != nil {
				m.udpMuxLn.Close()
				m.httpServer.close()
				ctxCancel()
	...
	if m.udpMuxLn != nil {
		str += ", " + m.PassiveUDPAddress + " (ICE/UDP)"
	}
	if m.tcpMuxLn != nil {
		str += ", " + m.PassiveTCPAddress + " (ICE/TCP)"
	}
...
func (m *webRTCManager) generateICEServers() ([]pwebrtc.ICEServer, error) {
	if m.Mode != conf.WebRTCModeICE {
		return []pwebrtc.ICEServer{}, nil
	}
